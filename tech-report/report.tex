...
\item Create a new \texttt{HyperWorkFlow} by calling \texttt{PhantomMetaHyperDagBuilder.build}, which calls \texttt{MetaHyperDagBuilder.build}, which adds add single
      hyperedge that represents all incoming meta-edges for all non-epsilon vertices and calls \texttt{HyperDagBuilder.build}, which constructs a new \texttt{HyperDag},
      shown in Figure \ref{hyperdag}.
